...
### 9. 🎯 스마트 카메라 오토포커스 & URL 딥링크 공유 (Deep Linking)
- 검색 자동완성이나 인스펙터 연관사 클릭 시 카메라가 해당 노드로 부드럽게 글라이딩 이동(`focusOnNode`).
- `?node={id}` 또는 `?view={mode}` 형태의 URL 파라미터를 지원하여, 특정 인물이나 기업 지배구조 화면을 동료에게 즉시 URL로 공유 가능.
- 모바일 및 태블릿 화면에서도 사이드바와 탭이 잘리지 않는 완전 반응형 UI 지원.
